      ...
      <div style={{ background: '#fff', borderRadius: 16, padding: 16, boxShadow: '0 1px 2px rgba(0,0,0,0.05)' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '0.85fr 1.15fr', gap: 16 }}>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
            ...
            {items.map((doc) => (
              <div key={doc.id} onClick={() => setSelectedId(doc.id)} style={{ border: '1px solid #e5e7eb', borderRadius: 12, overflow: 'hidden', minHeight: 120, cursor: 'pointer' }}>
                <div style={{ background: color(doc.status), color: '#fff', padding: '4px 10px', fontWeight: 700, fontSize: 13, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                  <span>Удалён</span>
                  <input type="checkbox" checked={selected.has(doc.id)} onChange={(e) => { e.stopPropagation(); toggle(doc.id) }} style={{ width: 16, height: 16 }} />
                </div>
                <div style={{ padding: 8 }}>
          ...
          <div style={{ background: '#f8fafc', border: '1px solid #e5e7eb', borderRadius: 12, padding: 12 }}>
            {selectedId && details ? (
              ...
              <div style={{ color: '#666' }}>Выберите документ слева, чтобы посмотреть историю и файл</div>
            )}
          </div>
        </div>
      </div>
